...
//Lava
#using scripts\zm\_zm_lava;

//Wallrunning
#using scripts\zm\_zm_wallrunning;

//Moving Platforms
#using scripts\zm\_zm_moving_platforms;

//Elevator
#using scripts\zm\_zm_elevator;
...
//Cleanup
#using scripts\zm\template_cleanup_mgr;

//Music
#using scripts\zm\_zm_music;
...
function main()
{
	...
	//Factory & ZOD Character
	if(isDefined(level.enable_all_characters) && level.enable_all_characters)
	{
		callback::on_spawned( &set_characters );
    	SetDvar( "enable_all_characters", 1 );
	}

    zm_tank::init();

    zm_minecart::minecart_init();

    zm_teleporter::teleporter_init();

    zm_ai_wasp::init();

    zm_ai_raps::init();

    zm_ai_keeper::keeper_spawn();

	zm_soul_box::soul_box_init();
	...
	//level thread zm_ai_astro::astro_zombie_spawning();

	template_mechz::enable_mechz_rounds();

    //zm_ai_quad::nova_crawlers_init();

	zm_moon_gravity::init();
	
	level thread zero_gravity();

    level thread zm_minecart::minecart_main();

    level thread add_powerups_after_round_1();

	level thread zmhd_powerswitch();

    level thread zm_teleporter::teleport_pad_hide_use();

    zm_weap_black_hole_bomb::blackhole_bomb_area_check();

    level thread setup_specialist();
	...
	level thread player_fell_off_map();

	//Character Voices
	if(isDefined(level.enable_zm_vox) && level.enable_zm_vox)
	{
		level thread add_zm_vox();
	}

	//Timed Gameplay
	if(isDefined(level.timed_gameplay) && level.timed_gameplay)
	{
		level thread timed_gameplay();
	}

	//Mid Round Dog Spawns
	level thread dogs_spawning_non_stop();

	level thread special_spawning_non_stop();
	...
	//Keep Perks After Last Stand & Death
	if(isDefined(level.keep_perks) && level.keep_perks)
	{
		callback::on_connect(&keep_perks);
	}

	//Hitmakers
	if(isDefined(level.enable_hitmakers) && level.enable_hitmakers)
	{
		zm::register_zombie_damage_override_callback(&zombie_damage_override_callback);
	}

	//Fire Sale
	if(isDefined(level.enable_firesale) && level.enable_firesale)
	{
		level thread enable_fire_sale();
	}

	//Special Monkey Rounds
	if(isDefined(level.special_monkey_rounds) && level.special_monkey_rounds)
	{
		level thread init_monkey_rounds();
	}

	//Temple Monkeys
	if(isDefined(level.temple_monkey) && level.temple_monkey)
	{
		zm_temple_ai_monkey::init();
	}
...
function offhand_weapon_overrride()
{
	...
	zm_utility::register_melee_weapon_for_level( "one_inch_punch" );
...
function offhand_weapon_give_override( weapon )
{
    ...
    if(weapon == level.w_quantum_bomb)
	{
	...
	{
		self zm_weap_nesting_dolls::player_give_nesting_dolls();
		self zm_weapons::play_weapon_vo(weapon);
		return true;
	}
	return false;
...
function zombie_damage_override_callback(willBeKilled, inflictor, attacker, damage, flags, meansofdeath, weapon, vpoint, vdir, sHitLoc, psOffsetTime, boneIndex, surfaceType)
{
    if (!IsDefined(attacker) || !IsPlayer(attacker))
        return;

    if(willBeKilled)
        attacker damagefeedback::update_override("damage_feedback", "mpl_hit_alert", "damage_feedback_glow_orange");
    else
        attacker damagefeedback::update(meansofdeath, inflictor, undefined, weapon, self, psOffsetTime, sHitLoc);
}

function characters_sort()
{
    zm_factory = [];
    for( i = 0; i < 3; i++ )
    {
    ...
    characterIndexArray = ArrayCombine( zm_factory, zm_zod, 0, 0 );
    characterIndexArray = array::randomize( characterIndexArray );

    return characterIndexArray;
}

function set_characters()
{    
    if( !IS_TRUE( GetDvarInt( "enable_all_characters" ) ) )
    {
        return;
    }

    self SetCharacterBodyType( characters_sort()[ self GetEntityNumber() ] );
}

function zmhd_powerswitch()
{
	elec_switch = getent("use_elec_switch", "targetname");
	zmhd_power_switch = struct::get("power_switch", "targetname");
	elec_switch setcursorhint("HINT_NOICON");
	elec_switch waittill("trigger", user);
	playsoundatposition("zmb_switch_flip", zmhd_power_switch.origin);
	playfx(level._effect["switch_sparks"], struct::get("elec_switch_fx", "script_noteworthy").origin);
	zmhd_power_switch scene::play("p7_fxanim_zmhd_power_switch_bundle");
}
...
function dogs_spawning_non_stop()
{
    level endon( "intermission" );
    
    n_start_spawning_from_round = 16; 
    n_minimum_zombie_total_for_spawn = 3;
    n_minimum_delay_between_spawns     = 10;
    n_maximum_delay_between_spawns     = 15;
    ...
    while ( 1 )
    {
        ...
        while ( n_count_total_zombie >= n_minimum_zombie_total_for_spawn )
        {
            wait randomIntRange( n_minimum_delay_between_spawns, n_maximum_delay_between_spawns ); 
            n_count_zombies_spawn = zombie_utility::get_current_zombie_count(); 
            n_count_total_zombie = n_count_zombies_spawn + level.zombie_total; 
            
            if( n_count_zombies_spawn < level.zombie_ai_limit && IS_TRUE( level.mixed_dog_rounds ) )
            { 
...
function init_monkey_rounds()
{
	zm_ai_monkey::special_monkey_rounds();
}

function init_temple_monkey()
{
	zm_temple_ai_monkey::init();
}

function special_spawning_non_stop()
{
    level endon( "intermission" );
    
    n_start_spawning_from_round = 2; 
    n_minimum_zombie_total_for_spawn = 3;
    n_minimum_delay_between_spawns     = 10;
    n_maximum_delay_between_spawns     = 15;
    ...
    while ( 1 )
    {
        ...
        while ( n_count_total_zombie >= n_minimum_zombie_total_for_spawn )
        {
            ...
            { 
                ...
                //zm_apothicon_fury::apothicon_fury_spawn();
                zm_ai_keeper::keeper_spawn();
                //zm_ai_raz::raz_spawn(1);
                //zm_ai_sentinel_drone::sentinel_drone_special_spawn();
                //zm_ai_thrasher::special_thrasher_spawn();
                //zm_ai_quad::quad_wave_init();
                //zm_ai_napalm::special_napalm_spawn();
            } 
        }
    }
}
...
{
	self thread zm_low_grav::LowGravity_PlayerHandle();
	self thread zero_gravity();
...
{
	if(!level.timed_gameplay)
		level waittill("timed_gameplay");
...
function player_fell_off_map()
{
	safety_net_trigs = GetEntArray("safety_net", "targetname");
	array::thread_all(safety_net_trigs, &watch_safety_net);
}

function watch_safety_net()
{
	level endon("kill_safety_net");

	save_points = struct::get_array(self.target, "targetname");

	while(1)
	{
		self waittill("trigger", player);

		foreach (place in save_points)
		{
			if(!PositionWouldTelefrag(place.origin))
			{
				player SetOrigin(place.origin);
...
{
	level.on_the_moon = 0;
	self thread zm_moon_gravity::zombie_moon_update_player_float();
}

function setup_specialist()
{
	a_triggers = getEntArray( "specialist_weapons", "targetname" );
	if ( !isDefined( a_triggers ) || a_triggers.size < 1 )
		return;
	
	for ( i = 0; i < a_triggers.size; i++ )
		a_triggers[ i ] thread specialist_weapon_trigger( getWeapon( a_triggers[ i ].script_string ) );
	
}

function specialist_weapon_trigger( w_weapon )
{
	...
	e_model thread specialist_powerup();
	...
	while ( 1 )
	{
		...
		e_player thread give_specialist( w_weapon, n_lua_index );
	}
}
...
function give_specialist( w_weapon, n_lua_index )
{	
	...
	self thread specialist_trigger_failsafe();
	...
	self clientfield::set_player_uimodel( "hudItems.hero_weapon_icon", n_lua_index );
...
function specialist_trigger_failsafe()
{
	self endon( "disconnect" );
	self.hero_taking = 1;
...
function enable_fire_sale()
{
	...
	while(isdefined(level.enable_mysterybox_at_every_location))
	{
		if(level.enable_mysterybox_at_every_location == true && (!isdefined(oldval) || oldval != level.enable_mysterybox_at_every_location))
		{
			...
			thread zm_powerup_fire_sale::toggle_fire_sale_on();
	
			for ( i = 0; i < level.chests.size; i++ )
				level.chests[ i ].zombie_cost = 950;
		}

		if(level.enable_mysterybox_at_every_location == false && (!isdefined(oldval) || oldval != level.enable_mysterybox_at_every_location))
		{
			oldval = level.enable_mysterybox_at_every_location;
...
function keep_perks()
{
	...
	while(isdefined(self))
	{
		while(level.keep_perks_after_downing && level.no_end_game_check && isdefined(self))
		{
			perk_list = [];
